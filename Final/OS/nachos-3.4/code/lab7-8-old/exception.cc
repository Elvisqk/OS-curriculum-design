...
//新增代码，PC推荐代码AdvancePC
void  AdvancePC() { 
    machine->WriteRegister(PCReg, machine->ReadRegister(PCReg) + 4); 
    machine->WriteRegister(NextPCReg, machine->ReadRegister(NextPCReg) + 4); 
} 

/*注释代码，原void ExceptionHandler(ExceptionType which)函数，只实现了Halt系统调用
void
...
// 修改代码，在此处实现异常处理函数完成系统调用void ExceptionHandler(ExceptionType which)
// $4-$7（4到7号寄存器）传递函数的前四个参数给子程序，参数多于4个时，其余的利用堆栈进行传递
void
ExceptionHandler(ExceptionType which)
{
    int type = machine->ReadRegister(2);
